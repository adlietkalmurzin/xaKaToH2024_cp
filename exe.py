...
def infer(list_path, ii, path_csv, thread):
    ...
    if len(pathes_to_imgs):
        ...
        with torch.no_grad():
            for i in range(len(pathes_to_imgs)):
                # Inference detector
...
class Ui(QtWidgets.QMainWindow):
    ...
    def predict_images(self):
        ...
        for id in os.listdir(self.selectedFolder):
            if os.path.isdir(os.path.join(self.selectedFolder, id)):
                print('dir')
                self.length += len(os.listdir(os.path.join(self.selectedFolder, id)))
        print(self.length)
        self.index = 0
    ...
    def finish(self, result):
        ...
        if result.get('finish') and not result.get('error'):
            self.selectedFolder = ""
            self.label.setText("Результаты сохранены в 'results.csv'")
            ...
            print(123123123123)
            self.index += 1
            self.progressBar.setValue(int((self.index / self.length * 100)))
